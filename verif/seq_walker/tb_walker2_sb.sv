// tb_walker2_sb.sv — IB-WALK stage-2 scoreboard TB for seq_layer_walker2
//                    (fmt=1 attention/projection/fetch subset).
//
...
module tb_walker2_sb;
  ...
  logic        lw_nsrc, lw_nsrc_own;   // E-4b: l_nsrc level + ownership
  logic        lw_gsrc;                // E-7: the gamma window level
  logic        nf_busy;                // E-3: the tile's LAYER_STATUS[5]
  logic        lu_valid, lu_ready, jc_valid, jc_ready, ljn_valid, ljn_ready;
  logic [1:0]  lu_unit;
  logic [11:0] lu_cols;
  logic [31:0] jc_data;
  logic [DIM_W-1:0] ljn_cols;
  logic        sj_valid, sj_ready;      // E-3b serializer job channel
  logic [7:0]  sj_beats;
  logic [3:0]  sj_lanes;
  ...
  seq_layer_walker2 #(.CFG_D(CFG_D), .N_ENG(N_ENG)) u_walk2 (
    ...
    .sj_valid(sj_valid), .sj_ready(sj_ready), .sj_beats(sj_beats),
    .sj_lanes(sj_lanes),
    .nf_busy(nf_busy),
  ...
  logic        eng_dbg_sc [N_ENG];
  logic        eng_dbg_sq [N_ENG];
  wire         _unused_dbg = &{1'b0, eng_dbg_sc[0], eng_dbg_sq[0]};

  for (genvar gi = 0; gi < int'(N_ENG); gi++) begin : g_comp
    logic req_v_i, res_r_i;
    assign req_v_i = cq_req_v && (int'(kv_eng_sel) == gi);
    assign res_r_i = cq_res_r && (int'(kv_eng_sel) == gi);
    seq_walker_comp #(.CFG_D(CFG_D)) u_comp (
      ...
      .dbg_stale_sc(eng_dbg_sc[gi]), .dbg_stale_sq(eng_dbg_sq[gi])
    );
  end

  assign cq_req_r    = eng_req_r[kv_eng_sel];
  assign cq_res_v    = eng_res_v[kv_eng_sel];
  assign cq_res_data = eng_res_d[kv_eng_sel];
  ...
  logic [7:0] stall_ds, stall_aj, stall_wj, stall_fj, stall_qj, stall_dj,
              stall_cs, stall_qs, stall_wf, stall_jc, stall_ljn, stall_sj;
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      stall_sj <= (sj_valid && sj_ready) ? new_stall()
                : (stall_sj != 0 ? stall_sj - 8'd1 : 8'd0);
    end
  end
  ...
  task automatic chk(input string got);
    ...
    begin
      n_checks++;
      if ($test$plusargs("trace_all"))
        $display("[GOT %0d] %s", n_checks, got);
  ...
  always begin
    ...
    if (rst_n) begin
      ...
      // E-3b: the serializer job (QSTAGE production framing)
      if (sj_valid && sj_ready)
        chk($sformatf("SERJ %0x %0x", sj_beats, sj_lanes));
    end
  end

  // LAYER level word: capture on change while walking (ROUTE/LCTL pattern).
  // E-3: composed through the PACKAGE function, so the TB cannot drift from
  // the walker's own bit map — bit 7 carries fsrc_ext[2] now, and a TB that
  // kept its hand-rolled `1'b0` there would have silently dropped the one
  // bit this change is about. E-4b: bit 19 carries l_nsrc the same way.
  logic [WALK2_LCTL_W-1:0] lctl_word, lctl_prev;
  logic                    lctl_seen;
  assign lctl_word = walk2_lctl(lw_rope_en, lw_rope_bank, lw_ser_dst,
                                lw_fsrc_ext, lw_resid_arm, lw_rope_pos,
                                lw_nsrc);

  // ── E-4b: the ownership contract, checked INDEPENDENTLY of the trace ─────
  // The scoreboard sees lw_nsrc through the level word; what it cannot see
  // is whether apex_top's mux would have TAKEN it. Three invariants:
  //   (a) the walker never RAISES lw_nsrc without ownership — at the tile
  //       that arm is a silent no-op (l_nsrc_q holds) and the walk wedges
  //       exactly the way E-4b exists to prevent. Edge-checked: the level
  //       legitimately HOLDS 1 across walk end (levels-hold doctrine).
  //   (b) ownership is never claimed by an image without EN_NSRC — that
  //       would clobber a host-armed l_nsrc under every LEGACY image. This
  //       is the check that fails an "own = walking" shortcut.
  //   (c) ownership never outlives the walk — it is a walk-scoped claim on
  //       apex_top's mux, not a level.
  logic lw_nsrc_d;
  always begin
    @(posedge clk);
    if (!rst_n) begin
      ...
      lw_nsrc_d <= lw_nsrc;
      if (lw_nsrc && !lw_nsrc_d && !lw_nsrc_own) begin
        $display("FAIL [%s]: lw_nsrc RAISED without ownership — the tile mux would ignore it",
                 cur_case);
        n_errors++;
      end
      if (lw_nsrc_own && !desc2_word[W2_MASK][W2_EN_NSRC]) begin
        $display("FAIL [%s]: l_nsrc ownership claimed by an image without EN_NSRC — legacy hold semantics broken",
                 cur_case);
        n_errors++;
      end
      if (lw_nsrc_own && !walk2_busy) begin
        $display("FAIL [%s]: l_nsrc ownership held outside a walk", cur_case);
        n_errors++;
      end
      // E-7 invariants — the gamma window is walk-scoped and FGAM-gated:
      // a window raised by an image without W2_EN_FGAM would steal xw
      // beats from the MXE on a legacy walk (the byte-identity breaker),
      // and one held outside a walk would wedge host-mode weight streams.
      if (lw_gsrc && !desc2_word[W2_MASK][W2_EN_FGAM]) begin
        $display("FAIL [%s]: gamma window raised by an image without EN_FGAM",
                 cur_case);
        n_errors++;
      end
      if (lw_gsrc && !walk2_busy) begin
        $display("FAIL [%s]: gamma window held outside a walk", cur_case);
        n_errors++;
      end
    end
  end

  // ── E-3: nf_busy stub (the tile's feed path, modelled) ──────────────────
  // apex_top's LAYER_STATUS[5] is high from the accepted C-1 feeder job
  // until the last element has reached the norm. Here: raised on the unit-3
  // LAYER_JOB accept, held for a randomized number of cycles so the walked
  // step cannot pass by sampling a lucky edge, then released. A walker that
  // failed to WAIT would race ahead of this window and desynchronize the
  // emission stream against the spec.
  int unsigned nf_cnt;
  assign nf_busy = (nf_cnt != 0);
  always_ff @(posedge clk) begin
    if (!rst_n) nf_cnt <= 0;
    else if (lu_valid && lu_ready && lu_unit == WALK2_LU_NORM)
      nf_cnt <= 3 + (rnd() % ((lat_mode == 0) ? 1 : 24));
    else if (nf_cnt != 0) nf_cnt <= nf_cnt - 1;
  end
  ...
  initial begin
    ...
    while ($fgets(line, fd) != 0) begin
      ...
      if (line.substr(0, 3) == "CASE") begin
        ...
        push_record(cur_grp, a, CFG_D, rec_vals);
    ...
    $display("WALKER2 RESULT: cases=%0d checks=%0d errors=%0d kvw=%0d heads=%0d mxelegal=%0d",
             n_cases, n_checks, n_errors, kv_writes, n_heads_seen, n_mxechk);
    if (n_errors == 0 && n_checks > 0) $display("WALKER2 PASS");
    else                               $display("WALKER2 FAIL");
    $finish;
  end
