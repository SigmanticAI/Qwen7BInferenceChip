// w4_job_sva.svh — job-level SVA pack for the B3 W4 weight feeder. Copied
// verbatim from verif/seam/seam_job_sva.svh (renamed only) so this lane owns
// its SVA include outright — no shared verif state with B1 / wide-RMSNorm
// (docs/design/B3_WEIGHT_PATH.md §7). Original header follows.
//
// REUSABLE job-level SVA pack for the D-021 seam blocks
...
`ifndef W4_JOB_SVA_SVH
`define W4_JOB_SVA_SVH
